...
export class PartnerWorkspaceComponent {
    ...
    return [
      {
        ...
        allLabel: 'All Supplier statuses',
        options: [
  ...
  get supplierFilterValues(): Readonly<Record<string, string>> {
    return {
      supplierStatus: this.supplierStatusFilter,
      periodStatus: this.supplierPeriodStatusFilter,
    }
  }
  ...
  get paymentFilterValues(): Readonly<Record<string, string>> {
    return {
      ...
      periodStatus: this.paymentPeriodStatusFilter,
    }
  }
  ...
  get filteredSuppliers(): readonly PartnerSupplierRow[] {
    ...
    return this.suppliers
      ...
      .filter(supplier => {
        ...
        const periodValues = this.periodsForSupplier(supplier).flatMap(period => [
          period.reference,
          period.periodStatus,
        ...
        return [supplier.business, supplier.identifier, supplier.status, ...periodValues]
          .join(' ')
          .toLowerCase()
          .includes(query)
      })
  ...
  get supplierTotalPages(): number {
    return Math.max(1, Math.ceil(this.filteredSuppliers.length / this.supplierPageSize))
  }

  get supplierPageNumbers(): readonly number[] {
    return Array.from({ length: this.supplierTotalPages }, (_, index) => index + 1)
  }

  get supplierRangeStart(): number {
    return this.filteredSuppliers.length ? (this.supplierPage - 1) * this.supplierPageSize + 1 : 0
  }

  get supplierRangeEnd(): number {
    return Math.min(this.supplierPage * this.supplierPageSize, this.filteredSuppliers.length)
  }
  ...
  get paymentTotalPages(): number {
    return Math.max(1, Math.ceil(this.filteredPaymentPeriods.length / this.paymentPageSize))
  }

  get paymentPageNumbers(): readonly number[] {
    return Array.from({ length: this.paymentTotalPages }, (_, index) => index + 1)
  }

  get paymentRangeStart(): number {
    return this.filteredPaymentPeriods.length ? (this.paymentPage - 1) * this.paymentPageSize + 1 : 0
  }

  get paymentRangeEnd(): number {
    return Math.min(this.paymentPage * this.paymentPageSize, this.filteredPaymentPeriods.length)
  }

  get paymentDueTotal(): number {
    return this.periods
      .filter(period => period.paymentStatusKey !== 'paid')
      .reduce((total, period) => total + period.amountToPay, 0)
  }

  get paymentDueCount(): number {
    return this.periods.filter(period => period.paymentStatusKey !== 'paid').length
  }

  get overduePaymentCount(): number {
    return this.periods.filter(period => period.paymentStatusKey === 'overdue').length
  }
  ...
  supplierForPeriod(period: PartnerPeriod): PartnerSupplierRow | undefined {
    return this.suppliers.find(supplier => supplier.id === period.supplierId)
  }

  activePeriodCount(supplier: PartnerSupplierRow): number {
    return this.periodsForSupplier(supplier)
      .filter(period => !['settled', 'expired'].includes(period.periodStatusKey))
      .length
  }

  supplierPaymentDue(supplier: PartnerSupplierRow): number {
    return this.periodsForSupplier(supplier)
      .filter(period => period.paymentStatusKey !== 'paid')
      .reduce((total, period) => total + period.amountToPay, 0)
  }
  ...
  completeUpload(): void {
    this.uploadOpen = false
    this.toast = 'Invoice batch received. Eligible invoices will update the matching Supplier periods.'
  }
  ...
  openSupplier(supplier: PartnerSupplierRow): void {
    this.closeDetailModals()
  ...
  openSupplierPeriod(period: PartnerPeriod): void {
    this.returnSupplier = this.selectedSupplier
    this.selectedSupplier = null
    this.selectedPeriod = period
  }

  openPaymentPeriod(period: PartnerPeriod): void {
    this.closeDetailModals()
    this.selectedPeriod = period
  }
  ...
  backToSupplier(): void {
    ...
    if (this.returnSupplier) this.selectedSupplier = this.returnSupplier
  ...
  onSupplierSearchValueChange(value: string): void {
    this.supplierSearch = value
    this.supplierPage = 1
  }

  onSupplierFilterValuesChange(values: Record<string, string>): void {
    this.supplierStatusFilter = values['supplierStatus'] ?? ''
    this.supplierPeriodStatusFilter = values['periodStatus'] ?? ''
    this.supplierPage = 1
  }

  changeSupplierPage(page: number): void {
    this.supplierPage = Math.min(Math.max(1, page), this.supplierTotalPages)
  }

  onPaymentSearchValueChange(value: string): void {
    this.paymentSearch = value
    this.paymentPage = 1
  }

  onPaymentFilterValuesChange(values: Record<string, string>): void {
    this.paymentStatusFilter = values['paymentStatus'] ?? ''
    this.paymentPeriodStatusFilter = values['periodStatus'] ?? ''
    this.paymentPage = 1
  }

  changePaymentPage(page: number): void {
    this.paymentPage = Math.min(Math.max(1, page), this.paymentTotalPages)
  }
