...
@Component({
  ...
  styles: [`
    ...
    .help-button{display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border:1px solid var(--av-color-surface-border,#d4dbe3);border-radius:50%;background:transparent;color:var(--av-color-text-muted,#66788a);font:600 12px inherit;cursor:pointer}
    ...
    .help-copy{position:fixed;z-index:10050;box-sizing:border-box;width:min(300px,calc(100vw - 24px));padding:12px 14px;border:1px solid #d4dbe3;border-radius:8px;background:#fff;color:#193540;box-shadow:0 5px 18px #1234;font-size:13px;font-weight:400;line-height:1.5;text-align:left;white-space:normal;pointer-events:none}
    @media(max-width:767px){.help-button{width:34px;height:34px}}
  `],
})
export class ReviewHelpComponent {
  ...
  show(): void {
    ...
    this.top = rect.bottom + 8 > window.innerHeight - 140 ? Math.max(12, rect.top - 130) : rect.bottom + 8
    this.open = true
  }
  ...
  @HostListener('document:click', ['$event']) outside(event: Event): void {
    if (!this.element.nativeElement.contains(event.target as Node)) { this.open = false; this.pinned = false }
  }
  @HostListener('document:keydown.escape') escape(): void { this.open = false; this.pinned = false }
}
