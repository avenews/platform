...
export class ContextualFinancingComponent implements OnDestroy {
  ...
  readonly sortOptions:readonly CustomerSortOption[]=[{value:'name-asc',label:'Name: A-Z'},{value:'available-desc',label:'Available financing: high to low'},{value:'available-asc',label:'Available financing: low to high'}]
  ...
  get filteredRelationships():readonly CustomerRelationship[]{const q=this.search.trim().toLowerCase();const items=(this.workspace?.relationships??[]).filter(r=>!this.status||(this.status==='available'?this.relationshipAvailable(r)>0:this.relationshipAvailable(r)<=0)).filter(r=>!q||[r.name,r.relationshipType,this.relationshipAvailabilityLabel(r),this.relationshipAvailable(r)].join(' ').toLowerCase().includes(q));return[...items].sort((a,b)=>{if(this.sort==='available-desc')return this.relationshipAvailable(b)-this.relationshipAvailable(a);if(this.sort==='available-asc')return this.relationshipAvailable(a)-this.relationshipAvailable(b);return a.name.localeCompare(b.name)})}
